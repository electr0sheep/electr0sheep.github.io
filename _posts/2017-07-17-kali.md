...
1. [Set up Kali on a thumb drive](#setting-up-kali-on-a-thumb-drive)
2. [Set up encrypted persistence mode](#setting-up-encrypted-persistence-mode)
3. [Install the wifi driver for MacBooks](#installing-the-wifi-driver-for-macbooks)
4. [Fix the tilde key on MacBooks](#fixing-the-tilde-key-on-macbooks)
5. [Set up an actual user account and disabling the root user](#setting-up-an-actual-user-account-and-disabling-the-root-user)
...
Next, immediately empty it and enjoy the satisfaction (I freed up almost a gig,
not sure what the heck was in there that was taking up that much space, probably
should have looked a little more carefully...)
